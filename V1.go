...
type Question struct {
	...
	WrongCount   int // Jumlah jawaban salah
}
...
func viewTopQuestions() {
	...
	case 2:
		// Sort questions by WrongCount in descending order
		sortedQuestions = make([]Question, len(questions))
		copy(sortedQuestions, questions)
		sort.Slice(sortedQuestions, func(i, j int) bool {
	...
	// Display the top 5 questions based on the selected criteria
...
func playQuiz(participant Participant) {
	fmt.Println("\n--- Starting Quiz ---")
	score := 0
	questionCount := 5 // Number of questions to ask in a quiz
	...
	for i := 0; i < questionCount && i < len(shuffledQuestions); i++ {
		...
		// Update the correct/incorrect counts
		if strings.ToUpper(answer) == q.Answer {
			score++
			questions[i].CorrectCount++ // Increment correct answer count
		} else {
			questions[i].WrongCount++ // Increment wrong answer count
		}
